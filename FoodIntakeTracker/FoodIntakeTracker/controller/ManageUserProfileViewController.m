...
- (void)viewDidLoad
{
    ...
    if ([appDelegate.loggedInUser admin].boolValue) {
        [self.rightView setFrame:CGRectMake(313, 52, 455, 880)];
    }
    else {
        [self.rightView setFrame:CGRectMake(0, 52, 768, 880)];
    }
...
- (IBAction)saveUpdateProfile:(id)sender {
    ...
    if(self.profileTable.hidden == NO && self.lblProfileFirstName.hidden == NO){
        ...
        NSString *imagePath = [Helper saveImage:UIImageJPEGRepresentation(self.imgProfilePhoto.image, 1.0)];
