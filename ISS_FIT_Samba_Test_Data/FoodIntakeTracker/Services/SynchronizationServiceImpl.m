...
>>>>>>> 7d183cd79eaceb537437987a93602b139f9bedb0
    ...
    if([usersToPush count] != 0 || [foodProductsToPush count] != 0 || [foodConsumptionRecordsToPush count] != 0) {
        ...
        [smbClient createDirectory:[NSString stringWithFormat:@"data_sync/%@",timestamp] error:&e];
        ...
        [smbClient createDirectory:[NSString stringWithFormat:@"data_sync/%@/data",timestamp] error:&e];
        CHECK_ERROR_AND_RETURN(e, error, @"Cannot create data directory.", SynchronizationErrorCode, YES, YES);
        
        [smbClient createDirectory:[NSString stringWithFormat:@"data_sync/%@/media",timestamp] error:&e];
...
<<<<<<< HEAD
            ...
            if (userToPush.profileImage && ![appDelegate.mediaFiles containsObject:userToPush.profileImage]) {                
                [smbClient createDirectory:[NSString stringWithFormat:@"data_sync/%@/media/%@", timestamp, userToPush.fullName] error:&e];
                CHECK_ERROR_AND_RETURN(e, error, @"Cannot create media directory.", SynchronizationErrorCode, YES, YES);
                isFolderCreated = YES;
...
>>>>>>> 7d183cd79eaceb537437987a93602b139f9bedb0
        ...
        for (AdhocFoodProduct* foodProductToPush in foodProductsToPush) {
            ...
            if (!isFolderCreated && additionalFiles.count > 0) {
                [smbClient createDirectory:[NSString stringWithFormat:@"data_sync/%@/media/%@", timestamp,
...
>>>>>>> 7d183cd79eaceb537437987a93602b139f9bedb0
        ...
        NSData *data = nil;
        data = [smbClient readFile:[NSString stringWithFormat:@"data_sync/%@/data/User.csv", subDirectory] error:&e];
        if (!data) {
            continue;
        }
...
<<<<<<< HEAD
        NSData *data = [smbClient readFile:[NSString stringWithFormat:@"data_sync/%@/data/AdhocFoodProduct.csv", subDirectory] error:&e];
=======
...
>>>>>>> 7d183cd79eaceb537437987a93602b139f9bedb0
        ...
        data = [smbClient readFile:[NSString stringWithFormat:@"data_sync/%@/data/FoodConsumptionRecord.csv", subDirectory] error:&e];
        ...
        data = [smbClient readFile:[NSString stringWithFormat:@"data_sync/%@/data/SummaryGenerationHistory.csv",
                          subDirectory] error:&e];
...
>>>>>>> 7d183cd79eaceb537437987a93602b139f9bedb0
        ...
        NSArray* dataFiles = [smbClient listFiles:[NSString stringWithFormat:@"data_sync/%@/%@/", subDirectory, folder] error:&e];
        if (!dataFiles) {
            // legacy data
            e = nil;
            folder = @"data";
            dataFiles = [smbClient listFiles:[NSString stringWithFormat:@"data_sync/%@/%@/", subDirectory, folder] error:&e];
            ...
            for (NSString *dataFile in dataFiles) {
                if ([dataFile hasSuffix:self.imageFileNameSuffix] || [dataFile hasSuffix:self.voiceRecordingFileNameSuffix]) {
                    ...
                    NSData *data = [smbClient readFile: [NSString stringWithFormat:@"data_sync/%@/%@/%@", subDirectory, folder, dataFile] error:&e];
            ...
            NSArray* folders = [smbClient listDirectories:[NSString stringWithFormat:@"data_sync/%@/%@/", subDirectory, folder] error:&e];
            for (NSString *innerFolder in folders) {
                dataFiles = [smbClient listFiles:[NSString stringWithFormat:@"data_sync/%@/%@/%@", subDirectory, folder, innerFolder] error:&e];
                for (NSString *dataFile in dataFiles) {
                    if ([dataFile hasSuffix:self.imageFileNameSuffix] || [dataFile hasSuffix:self.voiceRecordingFileNameSuffix]) {
                        NSString *localDataFile = [[DataHelper getAbsoulteLocalDirectory:self.localFileSystemDirectory]
                                                   stringByAppendingPathComponent:dataFile];
                        NSData *data = [smbClient readFile: [NSString stringWithFormat:@"data_sync/%@/%@/%@/%@", subDirectory, folder, innerFolder, dataFile] error:&e];
                        CHECK_ERROR_AND_RETURN(e, error, @"Cannot read image and recording file.",
                                               SynchronizationErrorCode, YES, NO);
                        [data writeToFile:localDataFile atomically:YES];
                    }
...
<<<<<<< HEAD
    ...
    [smbClient createDirectory:[NSString stringWithFormat:@"data_sync/%@/media",timestamp] error:&e];
    CHECK_ERROR_AND_RETURN(e, error, @"Cannot create media directory.",
                           SynchronizationErrorCode, YES, YES);
    ...
    // process SummaryGenerationHistory
    NSMutableData *summaryGenerationHistoryCSVData = [NSMutableData data];
    NSArray *summaryGenerationHistoriesToPush = [self getAllObjects:@"SummaryGenerationHistory" error:&e];
    CHECK_ERROR_AND_RETURN(e, error, @"Cannot fetch SummaryGenerationHistory to be pushed.",
                           EntityNotFoundErrorCode, YES, YES);
...
<<<<<<< HEAD
    float currentProgress = 0.8;
    float delta = 0;
    if (dataSyncSubDirectories.count > 0) {
        delta = 0.2 / dataSyncSubDirectories.count;
    }
=======
>>>>>>> 7d183cd79eaceb537437987a93602b139f9bedb0
    for (NSString *timestampDir in dataSyncSubDirectories) {
