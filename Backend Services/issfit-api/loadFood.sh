...
PSQL_ARGS="-X --echo-all --set AUTOCOMMIT=off --set ON_ERROR_STOP=on --single-transaction"
...
LOAD_CMD="copy food_tmp_table (name, categories, origin, barcode, fluid, energy, sodium, protein, carb, fat, image, deleted, version) FROM STDIN WITH (FORMAT 'csv', DELIMITER E',', HEADER, ENCODING 'UTF-8');"
